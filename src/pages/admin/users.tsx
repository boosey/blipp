...
import { useAdminFetch, useAdminGet } from "@/lib/api-client";
import type {
  AdminUser,
  AdminUserDetail,
  UserSegment,
...
import { AdminMasterDetail } from "@/components/admin/admin-master-detail";
...
export default function UsersPage() {
  ...
  const {
    ...
    setSelectedDetail,
    refresh,
  } = useAdminList<AdminUser, AdminUserDetail>({
    basePath: "/users",
  });

  const segment = (filters.segment as UserSegment) || "all";
          ...
          <div className="flex flex-wrap gap-1.5">
            {SEGMENT_FILTERS.map((sf) => {
              ...
              return (
                <button
                  key={sf.key}
                  onClick={() => setFilter("segment", sf.key)}
                  className={cn(
