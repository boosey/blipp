      ...
      try {
        ...
        if (!transcript) {
          ...
          for (let i = 0; i < modelChain.length; i++) {
            ...
            const keyResult = await resolveEnvForPipeline(prisma, env, "pipeline.stt", resolved.provider);
            ...
            if (!keyResult.fromDb && keyResult.envKey) {
              ...
              writeAiError(prisma, { service: "stt", provider: resolved.provider, model: providerModelId, operation: "key_resolution", correlationId: jobId, jobId, stepId: step.id, episodeId, category: "auth", severity: "permanent", errorMessage: `Env var fallback: ${keyResult.envKey} not configured in Service Keys DB for context pipeline.stt.${resolved.provider}`, requestDurationMs: 0, timestamp: new Date(), retryCount: 0, maxRetries: 0, willRetry: false }).catch(() => {});
            }
